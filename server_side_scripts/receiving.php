<?php
	...
	*/
	...
	// 	$sWhere .= "AND ";
	// }
	// $sWhere .= " logdel=0 AND isConformed=1 AND (isReceived=0 OR isReceived=2 OR isReceived=3)";
	...
	if ($status != '') {
		...
		if ($sWhere == "") {
			$sWhere .= "WHERE isReceived IN ($statusFilter) AND logdel = 0 AND isConformed=1";
		} else {
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
        ...
        if ($aRow['isReceived'] == 0) {
            $row[] = '<center><span class="badge bg-secondary text-light"><span>Pending</span></span><br><i style="font-size: 0.8em;">'.$aRow['date_time_created'].'</i></center>';
        } elseif ($aRow['isReceived'] == 1) {
            $row[] = '<center><span class="badge bg-success text-light"><span>Received<br><span> by: '. $aRow['receiver'] .'</span></span></span><br><i style="font-size: 0.8em;">'.$aRow['received_at'].'</i></center>';
        } elseif ($aRow['isReceived'] == 2) {
			$row[] = '<center><span class="badge bg-danger text-dark"><span>Disapproved</span></span></center>
			<center><button data-bs-toggle="modal" data-bs-target="#modalViewReceivingRevisionRemarks" class="btn btn-sm btn-danger btnRevision"
			...
			<center><button data-bs-toggle="modal" data-bs-target="#modalViewReceivingDisapproveReason" class="btn btn-sm btn-danger btnReason"
		...
		if($aRow['isReceived'] == 0) {
			...
			<button type="button" 
				...
				data-bs-target="#viewReceivingTraineeModal" 
				class="btn btn-success fa-solid fa-pen-to-square btn_Rview" 
				style="color:white"
				id="btn_Rview" 
				value="'.$aRow['pkid'].'" 
				...
				data-day2="'.$aRow['day2'].'" 
				data-day2_reason="'.$aRow['day2_reason'].'" 

				data-day3="'.$aRow['day3'].'" 
				data-day3_reason="'.$aRow['day3_reason'].'" 

				data-receive_status="'.$aRow['isReceived'].'" 
				>
			</button>';
		}
		elseif($aRow['isReceived'] == 1 || $aRow['isReceived'] == 2 || $aRow['isReceived'] == 3) {
			$row[]  = '<center>
			<button type="button" 
				data-bs-toggle="modal" 
				data-bs-target="#viewReceivingTraineeModal" 
				class="btn btn-info fa-solid fa-eye btn_Rview" 
				style="color:white"
				id="btn_Rview" 
				value="'.$aRow['pkid'].'" 
		...
		// 		data-bs-toggle="modal" 
		// 		data-bs-target="#viewReceivingTraineeModal" 
		// 		class="btn btn-info fa-solid fa-pen-to-square btn_Rview" 
		// 		style="color:white"
		// 		id="btn_Rview" 
		// 		value="'.$aRow['pkid'].'" 
