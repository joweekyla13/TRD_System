<?php
	...
	{
		$sOrder = "ORDER BY  ";
		$conformingCol = "isConformed";
	
		$sOrder .= $conformingCol . " DESC, ";
	...
	*/
	...
	// $sWhere .= " logdel=0 AND email='" . mysql_real_escape_string($username) . "'";

	// $sWhere .= " logdel=0 AND email='$username'";
    ...
    CASE WHEN fkdocno IN (SELECT docno FROM tbl_memo) THEN 1 ELSE 0 END AS in_memo
    FROM $sTable
    $sWhere
    GROUP BY conno
    $sOrder
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		// Edited 1-10-25
		if ($aRow['isConformed'] == 2 || $aRow['isReceived'] == 2 || $aRow['isHeadApproval'] == 2) {
			$row[]  = '<center>
				<button type="button" 
		...
		$badgeClass = ($aRow['in_memo'] == 1) ? 'badge bg-primary' : 'badge bg-dark';

		$row[] = '<center>' . $aRow['conno'] . '<br>' . '<span class="' . $badgeClass . '"><span>' . $aRow['fkdocno'] . '</span></span>' . '</center>';
        ...
        if ($aRow['isConformed'] == 0) {
            ...
            $row[] = '<center><span class="badge bg-success text-light"><span>Conformed<br><span>by: '.$aRow['approver'].'</span></span></span>
			<br><i style="font-size: 0.8em;">'.$aRow['conformed_at'].'</i>
			</center>';
        } elseif ($aRow['isConformed'] == 2) {
            $row[] = '<center><span class="badge bg-warning text-dark"><span>ForRevision</span></span></center>
			<center><button data-bs-toggle="modal" data-bs-target="#modalViewRevisionRemarks" class="btn btn-sm btn-warning btnRevision"
            ...
            $row[] = '<center><span class="badge bg-danger text-light"><span>Disapprove</span></span></center>
        ...
        if ($aRow['isReceived'] == 0) {
            ...
            $row[] = '<center><span class="badge bg-success text-light"><span>Received<br><span>by: '.$aRow['receiver'].'</span></span></span>
			<br><i style="font-size: 0.8em;">'.$aRow['received_at'].'</i>
			</center>';
