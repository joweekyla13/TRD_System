<?php
	...
	$aColumns = array('pkid','docno','date_time_created','classification','reason','memo_to','memo_from','subject','date_now','department','section','empNo','fullname','date_hired','pds','fromto','venue','endorsementDate','remarks','title','preparedby','notedby','status','revision','updated_at','approvedby');
	...
	*/
	...
	// 11/27/2024 start ng ng newly added code
	// Add status filter
	if ($status != '') {
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		elseif($aRow['status'] == 1 || $aRow['status'] == 2 || $aRow['status'] == 3 || $aRow['status'] == 4) {
			$row[]  = '<center>
			<button type="button" 
				data-bs-toggle="modal" 
				data-bs-target="#view_memo" 
		...
		if ($aRow['status'] == 0) {
            ...
            $row[] = '<center><span class="badge bg-success text-light"><span>Approved<br><span>by: '. $aRow['approvedby'] .'</span></span></span><br><i style="font-size: 0.8em;">'.$aRow['updated_at'].'</i></center>'; // Approved but w.o training
            ...
            $row[] = '<center><span class="badge bg-danger text-light"><span>Disapproved</span></span></center>
						...
						<br><i style="font-size: 0.8em;">'.$aRow['updated_at'].'</i></center>';
        } elseif ($aRow['status'] == 4) {
            $row[] = '<center><span class="badge bg-success text-light"><span>Approved<br><span>by: '. $aRow['approvedby'] .'</span></span></span><br><i style="font-size: 0.8em;">'.$aRow['updated_at'].'</i></center>'; // Approved but w training
        }
